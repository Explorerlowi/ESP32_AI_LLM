# ESP32+ASRPRO_AI_LLM
## 简介
本项目使用esp32接入讯飞星火大模型、豆包大模型（流式调用）、通义千问大模型实现语音对话聊天功能，同时接入asrpro模块，支持语音唤醒、连续对话、音乐播放、抽象整活等功能，同时外接了一块显示屏实时显示对话的内容。
## 使用说明
...
### 硬件使用清单
ESP-WROOM-32、INMP441全向麦克风、MAX98357 I2S音频放大器模块、喇叭、1.8寸（128x160）RGB_TFT屏幕、面包板（400孔85x55mm）两块、面包板跳线若干、数据线一条、asrpro开发板一块（2M4M均可）及其配套的数据线一条，led灯一个（可选）
...
### 功能介绍
#### 语音唤醒功能
通过asrpro唤醒，识别到唤醒词时，通过PA2输出一个低电平信号给19引脚，进入对话。
#### 语音对话功能
...
#### 抽象整活功能（在Dufs文件夹中）
开启dufs服务，配置好监听端口，esp32与开启dufs服务的设备需处于同一wifi下，使用唤醒词唤醒设备后，当设备处于待机状态或说话状态时，使用命令词丁真、坤坤、牢大即可触发对应的语音和音乐。
#### 其它指令
退下：进入待机状态（此时asrpro仍保持在唤醒状态），可通过打断命令词和boot按键进行对话，或者识别命令词丁真、坤坤、牢大，一个小时内未识别到唤醒词asrpro退出唤醒状态，此时需要重新唤醒才可进行命令词的识别
再见、拜拜、休眠：asrpro退出唤醒状态，此时只可通过boot按键进行对话
# 项目部署教程
- 下载vscode和platformIO插件，天问Block
- 开通讯飞相关服务（可选：开通豆包、通义千问大模型服务）
...
- 填写main.cpp中要求填写的讯飞账号参数（可选：填写豆包、通义千问大模型的参数）
- 安装esp32的驱动
- 编译、烧录
- 配置好asrpro模块
- 将所有元器件连接在一起
## 项目成品图参考
![image](./images/resultsv3.0.jpg)
![image](./images/resultsv3.0_1.jpg)
